...
# --------------------------------------------------
# Time-series augmentations for SimCLR
# --------------------------------------------------
...
class DLWFSimCLRDataset(Dataset):
...
class SimCLR1DEncoder(nn.Module):
    ...
    def forward(self, x):
        # input: (B, T, D) -> Conv1d expects (B, D, T)
        x = x.transpose(1, 2)
        x = self.stem(x)
        x = self.layer1(x)
        x = self.layer2(x)
...
class ProjectionHead(nn.Module):
    def __init__(self, in_dim: int, proj_hidden_dim: int, out_dim: int):
        super().__init__()
...
def train_simclr(model, train_loader, optimizer, scheduler, device, epochs, temperature, log_every=10):
    history = []
    model.train()
...
def main():
    parser = argparse.ArgumentParser(description="DLWF + SimCLR training")

    parser.add_argument("--data-path", type=str, default=DATA_PATH_DEFAULT)
    parser.add_argument("--save-dir", type=str, default="outputs_simclr_dlwf")
    ...
    parser.add_argument("--num-classes", type=int, default=2, help="Only keep the first K classes for a quick demo")
    parser.add_argument("--max-samples", type=int, default=100, help="Maximum number of total samples for demo")

    # SimCLR augmentation controls.
    ...
    ssl_train_dataset = DLWFSimCLRDataset(X_train, y_train, augmenter=augmenter)
    ...
    torch.save(
        ...
        os.path.join(args.save_dir, "simclr_encoder.pt"),
    )
